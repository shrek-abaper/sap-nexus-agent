...
当前代码已在 Runbooks 13-21 完成同快照 recall、PlanGraph v2、READ PlanExecutor、OutputProjection、RecommendationDecision、grounded Narrative、governed event/replay、Workbench integration，以及单 run owner HITL、完整 subject revalidation、durable exactly-once continuation 与 Gateway atomic claim。Runbook 22 的薄 TypeScript composition coordinator 已把这些组件接成 production runtime boundary，并以 fake/sandbox evidence 通过 L1/L2/L3 offline gate；这不等于 live SAP multi-READ 或 live SAP WRITE 已执行，也不证明本地 file/JSONL store 已达到 shared multi-worker/HA 量产要求。
...
当前 runtime 已实现 LLM-first hybrid intent、五态 `MatchDecision`、可靠 multi-intent escalation 与 deterministic PlanGraph v2；Runbook 22 coordinator 已在 offline fake/sandbox 边界接通 READ PlanExecutor、OutputProjection、建议、grounded narrative、durable replay 和单 Action continuation。live SAP composition 仍为 `not_run`。详细成熟度见 `docs/runbooks/README.md`；架构只保留下述契约和 fail-closed 边界。
...
| `ESCALATE_TO_PLANNER` | 用户目标明显需要多能力组合 | 生成受治理 PlanGraph candidate；仅 schema-valid、同 snapshot、无 blocking gap 的 handoff 可进入 Runbook 22 coordinator |
...
能力组合的通用 Dynamic Planner 仍是 `Reserved`。S1、S2-A、S2-B、P0B 与 Runbooks 13-22 已实现并归档；Runbook 22 已由薄 TypeScript coordinator 接通 READ PlanExecutor、OutputProjection、RecommendationDecision、grounded Narrative、Workbench replay 与 plan-aware single-user HITL Action continuation，并以 fake/sandbox evidence 通过 L1/L2/L3 offline gate。live SAP composition 仍未运行；不进入自由 runtime 编排。

组合不是单一概念，而是三种形态各异、支持前提不同的东西：

| 组合形态 | 语义 | 归属层 | 落地前提 |
|---|---|---|---|
| Fact-level 聚合 | 多个 `Function` 各产 `ReasoningFact`，由推理 / 叙事综合 | L6 / L7 | OutputProjection、RecommendationDecision、grounded Narrative 与 Workbench replay 已由 Runbook 22 coordinator 完成 offline 接线；live SAP 未运行 |
...
- 多能力请求已可靠落到 `ESCALATE_TO_PLANNER`，Runbook 22 已在 production coordinator boundary 完成 PlanGraph orchestration 的 offline 接线与 release-gate 证明；fake/sandbox evidence、UI label 和 fixture 仍不得推断为 live SAP execution。
