...
| `v0.1.3` | `2026-08-03` | 同步 S2-A/S2-B/P0B 已归档事实，并将原 S3 单一 pilot 拆分入口收口到 Runbooks 13-19；当前开发入口改为 Runbook 13，OpenHarness 仍仅作设计参考 | 当前决策基线 |
...
该范围已作为 `sap-nexus-planner-dry-run` 的第一 milestone 完成并归档，未另建执行 runtime。
...
已完成 change：`sap-nexus-planner-dry-run`
...
- P0B 已交付本地 durable Run/Approval 和 incremental SSE/reconnect；该 file/JSONL、单 worker 基线仍不是 multi-worker/HA 或生产 WRITE 的 readiness evidence。
...
| P0A Source-of-truth / repository hygiene | 已完成；不改变 runtime |
| S2-A Semantic MatchDecision Hardening | 已完成并归档；五态决策、多意图/歧义、visibility 和 matcher Eval |
| S2-B Planner Dry-run | 已完成并归档；progressive CapabilityCard + deterministic compiler；dry-run only |
| Trusted / Durable Runtime | P0B 四项基础已完成并归档；后续直接复用 |
| S3 Read-only Composition Pilot | Runbooks 13-19 已归档，生产 orchestrator 仍 deferred；不再作为单一活动 change |
| 自动本体/能力发布 | 禁止；只允许 draft + human publish |
| Dynamic Planner | Phase 3+ / Reserved |
| Write composition | Reserved；保持逐节点审批和事务边界 |

P0A、S2-A/S2-B、P0B 和 Runbooks 13-19 均已完成并归档。当前从 Runbook 20 开始，按 `20 -> 21 -> 22` 完整交付；OpenHarness 继续只提供机制参考，不成为 runtime、执行权威或自由 Tool Calling 入口。
