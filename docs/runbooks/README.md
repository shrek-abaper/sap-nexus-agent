...
Current completed change = sap-nexus-read-plan-executor (Runbook 16 implemented 2026-08-04 at openspec/changes/sap-nexus-read-plan-executor/): READ-only PlanExecutor consuming validated PlanGraph v2 readPartition; PlanGraphV2Parser + NodeStateMachine + DurableNodeLedger + DagScheduler + FakeGateway + SseEmitter + PlanExecutor with timeout/cancel/restart-recovery/idempotent-replay/dependency-blocking/lease-conflict-fail-closed; Action nodes BLOCKED_APPROVAL; 954 pytest / 174 frontend tests / 19 openspec pass; production orchestrator wiring deferred to Runbook 17
Planned sequence = archived semantic planning foundation -> S2-A/S2-B archived -> conversation/batch archived -> P0B four-part foundation archived -> runbook 13 governed context archived -> runbook 14 recall archived -> runbook 15 PlanGraph v2 archived -> runbook 16 READ PlanExecutor implemented -> runbooks 17-22 complete Agent delivery
...
Current composition runtime = PlanExecutor implemented (READ-only DAG scheduling, durable node ledger, timeout/cancel/recovery/idempotency); OutputProjection, recommendation and multi-capability business composition still not implemented; production orchestrator wiring deferred to Runbook 17
...
| `Planned` | 完整 Agent Runbooks 14-22 | LLM-first 召回、PlanGraph v2、READ executor、组合事实、建议、grounded narrative、Workbench、单 Action 人审闭环和 E2E release gate |
...
- 第二条 live read capability、sandbox write pilot、S1、S2-A/S2-B、会话/批量能力、P0B 和 Runbook 13 受治理上下文均已完成；下一实施入口固定为 runbook 14，不直接跳到 PlanExecutor 或 WRITE。
...
| `16` | `16-read-plan-executor.md` | `v0.1.0` | Implemented / Archived | `2026-08-04` | Execute validated READ nodes with ready-node scheduling and durable ledger; design `openspec/changes/sap-nexus-read-plan-executor/design.md` / plan `openspec/changes/sap-nexus-read-plan-executor/tasks.md` / change `openspec/changes/sap-nexus-read-plan-executor/`; 954 pytest / 174 frontend tests / 19 openspec pass |
