...
- [x] 1.2 Node 侧实现 PlanGraph v2 反序列化与 `readPartition` / 节点 / edges / `snapshotId` 解析
- [x] 1.3 校验 PlanGraph v2 有效性 + `snapshotId` 未漂移，无效/漂移 fail-closed 并记录结构化失败
...
- [x] 5.1 实现节点级超时 -> `TIMED_OUT`，不阻塞独立节点
- [x] 5.2 实现用户取消 -> 未完成节点 `CANCELLED`，`SUCCEEDED` 保留
